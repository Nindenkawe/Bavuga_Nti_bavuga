...
class ChallengeGeneratorProcessor(processor.Processor):
    def __init__(self, model_names: list[str], image_dir: str = "sampleimg"):
        ...
        self.prompts = {
            ...
            "riddle_hint": (
                ...
                "Provide a short, one-sentence hint or description for this riddle in English. "
            ...
            "image_prompt_generation": (
                ...
                "The prompt should describe a new, unique, and visually interesting scene in Rwanda, inspired by the provided image but not a direct copy. "
    ...
    async def _run_image_generation_processor(self, prompt: str) -> bytes:
        for model_name in self.model_names:
    ...
    async def generate_hint(self, riddle: str) -> dict:
        """Generates a hint for a given riddle."""
        try:
            processor_input = {"riddle": riddle}
    ...
    async def _generate_challenge_logic(self, difficulty: int, state: GameState, game_mode: str) -> dict:
        try:
            ...
            # --- Thematic Linking ---
                ...
                english_phrase = await self._run_text_processor(processor_input, "themed_sentence")
                ...
                kinyarwanda_translation = await self._run_text_processor(translation_input, "translate_to_kinyarwanda")
            ...
            # --- Standard Challenge Generation ---
                ...
                if game_mode == "sakwe":
                    ...
                    try:
                        ...
                        # 1. Generate a detailed prompt for the new image
                        prompt_generation_input = {"image": img}
                        ...
                        # 2. Generate the new image using the prompt
                        generated_image_data = await self._run_image_generation_processor(image_prompt)
